...
import { X, BookOpen, Hash, Award, Plus } from 'lucide-react';
import type { AddCourseData } from '../hooks/useCourses';

interface AddCourseModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSubmit: (courseData: AddCourseData) => Promise<{ data: any; error: string | null }>;
  loading?: boolean;
}

export const AddCourseModal = ({ isOpen, onClose, onSubmit, loading = false }: AddCourseModalProps) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!error) {
        ...
        setFormData({ course_name: '', course_code: '', credits: undefined });
        setErrors({});
        onClose();
      } else {
                  ...
                  <form onSubmit={handleSubmit} className="space-y-6">
                    ...
                    <div className="space-y-2">
                      ...
                      <input
                        ...
                        placeholder="Data Structres & Algroithms"
                        disabled={isSubmitting}
                        className={cn(
