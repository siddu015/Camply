...
CRITICAL: When routing to sub-agents, you MUST pass the user_id from the fetched user context. Your request must be a single string like this: 
- Campus: "Fetch information about [topic] for user_id: [user_id_uuid]"
- Handbook: "Answer handbook question: [question] for user_id: [user_id_uuid]" or "Process handbook for handbook_id: [extracted_handbook_id] for user_id: [user_id_uuid]"
...
- Route handbook queries to the handbook_agent with proper user_id and extracted handbook_id when applicable
- Route syllabus queries to the syllabus_agent with proper user_id and extracted course_id when applicable
"""