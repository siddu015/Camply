...
async def analyze_prompt_based_intelligence(prompt_id: str, custom_prompt: str = "", *, tool_context) -> Dict[str, Any]:    
    try:
        # Handle different types of session state access
        ...
        college_name = college_info.get('name') if isinstance(college_info, dict) else None
        college_id = academic_details.get('college_id') if isinstance(academic_details, dict) else None
        ...
        if not college_name or not college_id:
            return {
                ...
                "debug_info": {
                    ...
                    "academic_details_type": type(academic_details).__name__,
                    "college_info_type": type(college_info).__name__
                }
            }
        ...
        if prompt_id == "custom" and custom_prompt:
            analysis_result = await analyze_custom_prompt(
                custom_prompt, college_name, user_data, cached_content
            )
        elif prompt_id in CONTENT_TYPE_MAPPING:
            analysis_result = await analyze_predefined_prompt(
                prompt_id, college_name, user_data, cached_content
            )
        else:
            return {
                "success": False,
        ...
        return {
            ...
            "metadata": {
                ...
                "user_personalized": bool(academic_details.get('branch_name')),
...
async def analyze_predefined_prompt(prompt_id: str, college_name: str, user_data: Dict[str, Any], cached_content: Optional[Dict] = None) -> Dict[str, Any]: 
    academic_details = user_data.get('user_academic_details', {})
...
async def analyze_custom_prompt(custom_prompt: str, college_name: str, user_data: Dict[str, Any], cached_content: Optional[Dict] = None) -> Dict[str, Any]: 
    academic_details = user_data.get('user_academic_details', {})
    department = academic_details.get("department_name", "")
    branch = academic_details.get("branch_name", "")
...
def format_dict_content(cached_content: Dict) -> str:
    ...
    if isinstance(cached_content, str):
        ...
        formatted_lines = []
        for key, value in cached_content.items():
    ...
    elif isinstance(cached_content, list):
        return "\n".join([f"• {item}" for item in cached_content])
    else:
        return str(cached_content)

def extract_sections_from_content(cached_content: Dict, prompt_id: str) -> List[str]:
